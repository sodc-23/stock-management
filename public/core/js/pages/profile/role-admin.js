$(document).ready(function(){
	...
	$("#add-profile").click(function(e){
		e.preventDefault();
		notification('<strong>ADD NEW</strong>',$("#profile-container").html());
	});
	...
	$('#delete-profile').click(function(e){
		e.preventDefault();
		var custom ='<form action="'+$("body").attr("data-link")+'/profile/profile-engine/delete" class="ejex_form" method="post" data-type="json" data-onsuccess="delete_profile" data-custom-message="Cheers! the profile has been successfully deleted." data-success-function="hide_pokemon">'+
					'<input type="hidden" name="type" value="delete">'+
					'<input type="hidden" name="id" value="'+$.trim($('#profile [data-identity="username"]').text())+'">'+
					'<fieldset>'+
					'<div class="text_align_center">Are you sure you want to delete this profile? click DELETE to proceed.</div>'+
					'</fieldset>'+
					'<div class="display_table center margin_top10px">'+
					'<button class="btn future-button margin_zero"><div class="display_table"><div class="display_row"><div class="display_cell padding_right5px"><i class="ion-android-delete"></i></div><div class="display_cell">DELETE</div></div></div></button>'+
					'</div>'+
					'</form>';
		notification('',custom);
	})
...
function delete_profile(e){
	if(e.success){
		$('#select-employee-profile option[value="'+$.trim($('#profile [data-identity="username"]').text())+'"]').remove();
		$('#select-employee-profile').next('.future-select-ui').find('li a[data-value="'+$.trim($('#profile [data-identity="username"]').text())+'"]').parent().remove();
		if(e.current_user_info.img){
			$("#profile-image-container").html('<img src="data:image/jpeg;base64,'+e.current_user_info.img+'" class="radius_circle bg_gray" style="width:100px;height:100px;display:block;">');
		}else{
			$("#profile-image-container").html('<div class="radius_circle bg_gray text_align_center" style="width:100px;height:100px;line-height:100px;display:block;"><i class="ion-person" style="font-size:60px;"></i></div>');
		}
		$('#profile [data-identity="username"]').text(e.current_user_info.username);
		$('#profile [data-identity="role"]').text(e.current_user_info.role);
		$('#profile [data-identity="password"]').text(e.current_user_info.password);
		$('#profile [data-identity="fullname"]').text(e.current_user_info.fullname);
		$('#profile [data-identity="age"]').text(e.current_user_info.age);
		$('#profile [data-identity="address"]').text(e.current_user_info.address);
		$('#profile [data-identity="marital status"]').text(e.current_user_info.marital_status);
		$('#profile [data-identity="email"]').text(e.current_user_info.email);

		$('#select-employee-profile').val(e.current_user_info.username);
		$('#select-employee-profile').next(".future-select-ui").find('.j_text').text('Your profile');
	}
}